...
import (
	...
	conspec "github.com/blckit/go-spec-consensus"
)
...
type Consensus struct {
	...
	ConsensusSpec conspec.ConsensusSpec

	// The spec used to determine which blocks are in play
	CompetitionSpec conspec.CompetitionSpec

	// Currently active blocks in the consensus finder
	blocks map[string]blkspec.Block

	// Current known heads in the consensus finder
	heads []string

	// blkspec.Blocks already submitted for a given parent block
...
func NewConsensus(consensusSpec conspec.ConsensusSpec, competitionSpec conspec.CompetitionSpec) *Consensus {
	...
	c.blocks = make(map[string]blkspec.Block)
...
// AddBlock method.
func (c *Consensus) WasSeen(block blkspec.Block) bool {
...
func (c *Consensus) SetCompeted(head blkspec.Block) {
	headID := head.GetID()
...
// children branches are pruned according to the rules in the blkspec.BlockComparator
...
func (c *Consensus) AddBlock(block blkspec.Block) (added bool) {
	if c.WasSeen(block) {
		return false
	}
	c.setSeen(block)
...
func (c *Consensus) GetBestBranch() []blkspec.Block {
	c.Lock()
	defer c.Unlock()
...
func (c *Consensus) removeUnfavorables(blocks []blkspec.Block) []blkspec.Block {
	if len(blocks) < 2 {
		return blocks
	}
...
func (c *Consensus) getFavorables(blocks []blkspec.Block) []blkspec.Block {
	if len(blocks) < 2 {
		return blocks
	}
	...
	for i < len(bs)-1 {
		...
		case conspec.ComparatorResultKeepFirst:
			bs = append(bs[:j], bs[j+1:]...)
			// jth element is already the next block, so don't increment j
		case conspec.ComparatorResultKeepSecond:
			bs = append(bs[:i], bs[i+1:]...)
			j = i + 1 // reset j since ith element is now the next block
		case conspec.ComparatorResultKeepNeither:
			bs = append(bs[:i], bs[i+1:]...)
			bs = append(bs[:j], bs[j+1:]...)
			j = i + 1 // reset j since ith element is now the next block
		case conspec.ComparatorResultKeepBoth:
			j++	
		}
	...
	result := make([]blkspec.Block, len(bs))
	for i, x := range bs {
		result[i] = x.block
	}
...
func (c *Consensus) getBranchMaxBlockNumber(block blkspec.Block) uint64 {
...
func (c *Consensus) getBranch(block blkspec.Block) []blkspec.Block {
	this := []blkspec.Block{block}
...
func (c *Consensus) setSeen(block blkspec.Block) {
	parentID := block.GetParentID()
...
func (c *Consensus) getParent(blockID string) blkspec.Block {
...
func (c *Consensus) getChildren(parentBlockID string) []blkspec.Block {
	children := make([]blkspec.Block, 0)
...
func (c *Consensus) elminateBlock(block blkspec.Block) {
	blockID := block.GetID()
